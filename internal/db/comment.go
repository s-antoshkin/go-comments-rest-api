...
func (d *Database) GetComment(
	...
	if err != nil {
		return comment.Comment{}, fmt.Errorf("erorr fetching the comment by uuid: %v", err)
	}
...
func (d *Database) PostComment(ctx context.Context, cmt comment.Comment) (comment.Comment, error) {
	...
	if err != nil {
		return comment.Comment{}, fmt.Errorf("failed to insert comment: %v", err)
	}
...
func (d *Database) DeleteComment(ctx context.Context, id string) error {
	...
	if err != nil {
		return fmt.Errorf("failed to delete comment from database: %v", err)
	}
...
func (d *Database) UpdateComment(
	...
	if err != nil {
		return comment.Comment{}, fmt.Errorf("failed to update comment: %v", err)
	}
